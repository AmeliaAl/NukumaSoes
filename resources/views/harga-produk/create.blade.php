    <div class="py-12 min-h-screen bg-[url('/images/background_fluid.png')] bg-cover bg-center bg-fixed relative flex items-center justify-center">
        ...
        <div class="w-full max-w-2xl px-6 relative z-10">
            ...
            <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-10">
                <form method="POST" action="{{ route('harga-produk.store') }}" class="space-y-8">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <!-- Kategori ID -->
                        <div>
                            <label for="kategori_id" class="block text-sm font-bold text-[#d4af37] mb-3 uppercase tracking-wider">Kategori ID <span class="text-red-500">*</span></label>
                            <div class="relative group">
                                <select name="kategori_id" id="kategori_id" 
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" data-nama="{{ $category->nama_kategori }}" {{ old('kategori_id') == $category->id ? 'selected' : '' }}>
                                            {{ $category->id }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <!-- Nama Kategori -->
                        <div>
                            <label for="nama_kategori_display" class="block text-sm font-bold text-[#d4af37] mb-3 uppercase tracking-wider">Nama Kategori</label>
                            <input type="text" id="nama_kategori_display" 
                                   class="w-full bg-white/80 border-2 border-white/30 rounded-2xl px-5 py-4 text-gray-700 font-medium placeholder-gray-400 shadow-inner leading-normal" 
                                   readonly placeholder="Pilih ID untuk melihat nama">
                        </div>
                    </div>

                    <!-- Jenis Mitra -->
                    <div>
                        <label for="jenis_mitra" class="block text-sm font-bold text-[#d4af37] mb-3 uppercase tracking-wider">Jenis Mitra <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <select name="jenis_mitra" id="jenis_mitra" 
                                ...
                                <option value="Agen" {{ old('jenis_mitra') == 'Agen' ? 'selected' : '' }}>Agen</option>
                                <option value="Riseller" {{ old('jenis_mitra') == 'Riseller' ? 'selected' : '' }}>Riseller</option>
                                <option value="Konsinyasi" {{ old('jenis_mitra') == 'Konsinyasi' ? 'selected' : '' }}>Konsinyasi</option>
                                <option value="Umum" {{ old('jenis_mitra') == 'Umum' ? 'selected' : '' }}>Umum</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                    ...
                    <!-- Harga -->
                    <div>
                        <label for="harga" class="block text-sm font-bold text-[#d4af37] mb-3 uppercase tracking-wider">Harga <span class="text-red-500">*</span></label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                <span class="text-gray-400 font-medium">Rp</span>
                            </div>
                            <input type="number" name="harga" id="harga" value="{{ old('harga') }}" step="0.01" min="0"
