...
@section('title', 'Input Biaya Overhead')
@section('page-title', 'Input Biaya Overhead Pabrik')

@section('content')
<div class="page-header">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h4 class="mb-1">Input Biaya Overhead Pabrik</h4>
            <p class="text-muted mb-0">Input biaya overhead pabrik untuk job order</p>
        </div>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('biaya-overhead-pabrik.store') }}" method="POST">
            @csrf
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" 
                               ...
                               value="{{ old('tanggal_overhead', now()->format('Y-m-d')) }}"
                               required>
                        ...
                        <select class="form-select @error('id_kategori_bop') is-invalid @enderror" 
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id_kategori_bop }}" 
                                        ...
                                        {{ old('id_kategori_bop') == $cat->id_kategori_bop ? 'selected' : '' }}>
                                    {{ $cat->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('satuan_periode') is-invalid @enderror" 
                                ...
                                id="satuanPeriode"
                                required>
                            <option value="per_batch" {{ old('satuan_periode', 'per_batch') == 'per_batch' ? 'selected' : '' }}>Per Batch</option>
                            <option value="per_hari" {{ old('satuan_periode') == 'per_hari' ? 'selected' : '' }}>Per Hari</option>
                            <option value="per_minggu" {{ old('satuan_periode') == 'per_minggu' ? 'selected' : '' }}>Per Minggu</option>
                            <option value="per_bulan" {{ old('satuan_periode') == 'per_bulan' ? 'selected' : '' }}>Per Bulan</option>
                        </select>
                        <small class="text-muted">Otomatis sesuai jenis, bisa diubah</small>
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        <label class="form-label" id="jumlahPeriodeLabel">Jumlah Batch <span class="text-danger">*</span></label>
                        <input type="number" 
                               class="form-control @error('jumlah_periode') is-invalid @enderror" 
                               name="jumlah_periode" 
                               id="jumlahPeriode"
                               value="{{ old('jumlah_periode', 1) }}"
                               min="1"
                               step="1"
                               required>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" 
                                                   ...
                                                   value="{{ old('total_nominal_global') }}"
                                                   min="0"
                                                   step="1000">
                                        </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" 
                                               ...
                                               value="{{ old('jumlah_batch_terlibat', $totalBatchAktif) }}"
                                               min="1"
                                               step="1">
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-2"></i>Simpan Biaya Overhead
                </button>
                <a href="{{ route('biaya-overhead-pabrik.index') }}" class="btn btn-secondary">
...
@section('scripts')
<script>
    // Default satuan_periode berdasarkan jenis overhead
    const defaultSatuan = {
        'Listrik': 'per_bulan',
        'Air': 'per_bulan',
        'Gas': 'per_hari',
    ...
    document.getElementById('jenisOverhead').addEventListener('change', function() {
        ...
        if (defaultSatuan[jenis]) {
            document.getElementById('satuanPeriode').value = defaultSatuan[jenis];
            updateLabels();
        }
    ...
    // Update labels saat satuan_periode berubah
    ...
    function calculateTotal() {
        ...
        // Jika pembagian biaya aktif dan kita pakai proporsi, maka nominal sudah mewakili total porsi untuk jumlah_batch
        // Tetapi karena sistem sebelumnya nominal dikalikan jumlah_periode, kita perlu memastikan:
        // Jika shared BOP dipakai, "nominal" di atas sebenarnya adalah total biaya, atau biaya per-batch.
        // Agar sesuai dengan struktur lama, kita anggap nominal yang didapat dari btnHitungProporsi adalah TOTAL, 
        // lalu kita bagi dengan jumlah batch untuk ditaruh di kolom nominal.
