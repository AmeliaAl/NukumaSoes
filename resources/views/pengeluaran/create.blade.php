                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('pengeluaran.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="produk_expired" id="produk_expired" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <option value="">-- Pilih Produk Expired --</option>
                                @foreach($expiredProducts as $product)
                                    <option value="{{ $product->nama_produk }} (Batch: {{ $product->no_batch }})" 
                                            data-cost="{{ $product->harga_pokok_per_pack }}"
                                            {{ old('produk_expired') == $product->nama_produk ? 'selected' : '' }}>
                                        {{ $product->nama_produk }} - Batch: {{ $product->no_batch }} (Exp: {{ \Carbon\Carbon::parse($product->tgl_expired)->format('d/m/Y') }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="jumlah_expired" id="jumlah_expired" value="{{ old('jumlah_expired') }}" 
                            ...
                            <div>
                                ...
                                <div class="relative mt-1">
                                    ...
                                    <input type="number" name="harga_pokok_per_pack" id="harga_pokok_per_pack" value="{{ old('harga_pokok_per_pack') }}" step="0.01" 
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const productSelect = document.getElementById('produk_expired');
                                ...
                                productSelect.addEventListener('change', function() {
                                    ...
                                    if (cost > 0) {
                                        hargaInput.value = cost.toFixed(2);
                                        calculateNominal();
                                    }
                                });
