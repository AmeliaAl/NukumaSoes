        ...
        @forelse($entries as $entry)
            ...
            @if(isset($entry['is_saldo_akhir']) && $entry['is_saldo_akhir'])
                <tr>
                    ...
                    <td style="background-color: #e5e7eb; color: #000000; font-weight: bold; text-align: right;">{{ ($entry['saldo'] < 0 ? '-' : '') . 'Rp ' . number_format(abs($entry['saldo']), 0, ',', '.') }}</td>
                </tr>
            @else
                <tr>
                    ...
                    <td style="text-align: right;">{{ ($entry['saldo'] < 0 ? '-' : '') . 'Rp ' . number_format(abs($entry['saldo']), 0, ',', '.') }}</td>
                </tr>
            @endif
        @empty
        <tr>
