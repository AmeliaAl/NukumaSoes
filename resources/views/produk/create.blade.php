        ...
        <div class="w-full max-w-4xl px-6 relative z-10">
            ...
            <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[40px] shadow-[0_30px_60px_rgba(0,0,0,0.4)] p-10">
                <div class="mb-10 text-center">
                    <h2 class="text-3xl font-serif font-black text-[#d4af37] drop-shadow-lg uppercase tracking-widest">Tambah Produk Baru</h2>
                    <p class="text-white/60 text-xs font-bold mt-2 tracking-[0.2em]">PENGISIAN MASTER DATA PRODUK & VARIAN</p>
                </div>

                <form method="POST" action="{{ route('produk.store') }}" class="space-y-8">
                    @csrf
                        ...
                        <div class="relative">
                            <select name="kategori" id="kategori" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->nama_kategori }}" data-harga="{{ $category->harga }}" data-masasimpan="{{ $category->masa_simpan }}" {{ old('kategori') == $category->nama_kategori ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="jenis_produk" id="jenis_produk"
                                ...
                                <option value="Brand Sendiri" {{ old('jenis_produk') == 'Brand Sendiri' ? 'selected' : '' }}>Brand Sendiri</option>
                                <option value="Maklon" {{ old('jenis_produk') == 'Maklon' ? 'selected' : '' }}>Maklon</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="hpp" id="hpp" value="{{ old('hpp') }}" 
                    ...
                    <!-- Hidden Fields -->
                    <input type="hidden" name="harga_jual" id="harga_jual" value="{{ old('harga_jual', 0) }}">
                    <input type="hidden" name="masa_simpan" id="masa_simpan" value="{{ old('masa_simpan', 0) }}">
                    <input type="hidden" name="satuan_masa_simpan" value="hari">
